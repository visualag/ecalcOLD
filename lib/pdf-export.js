...
// Generează un PDF stilizat în format A4 - "Fluturași de Salariu"
import { jsPDF } from 'jspdf';

export const generateSalaryPDF = (result, year = 2026) => {
  // Creăm un document A4
  const doc = new jsPDF({
  ...
  doc.setFontSize(22);
  doc.text('eCalc.ro', margin, 18);
  
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('Calculatoare Fiscale Profesionale', margin, 26);
  
  doc.setFontSize(12);
  doc.text(`Fluturași de Salariu - ${year}`, pageWidth - margin - 70, 22);
