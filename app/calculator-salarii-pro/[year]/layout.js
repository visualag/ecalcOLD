...
export async function generateMetadata({ params }) {
  ...
  if (slug) {
    ...
    if (slug.includes('net-brut')) isNetToBrut = true;
  }
  ...
  return {
    ...
    alternates: {
      canonical: slug ? `/calculator-salarii-pro/${year}/${slug}` : `/calculator-salarii-pro/${year}`
    },
    openGraph: {
      title: displayTitle,
      description: displayDesc,
      url: `https://ecalc.ro/calculator-salarii-pro/${year}${slug ? '/' + slug : ''}`,
      type: 'article',
    },
    twitter: {
